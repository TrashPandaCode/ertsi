...
def main():
    ...
    folder = "recordings/zw8-12/20250430_123907"
    timestamp = "20250430_123907"
    ...
    rec_signal = pf.io.read_audio(
        "recordings/zw8-12/20250430_123907/recording_20250430_123907.wav")
    ...
    ir_processed = pf.dsp.time_window(ir_processed, [
                                      0, .01, 0.9, 1], unit='s', crop='window')  # adjust window if needed
    ...
    for center_freq in bands:
        filtered_ir = pf.dsp.filter.butterworth(
            ir_processed, 4, [center_freq/np.sqrt(2), center_freq*np.sqrt(2)], 'bandpass')
        rt60_band = pra.experimental.measure_rt60(
            filtered_ir.time.T, fs, plot=True)
        band_rt60s[center_freq] = rt60_band

        plot_filename = os.path.join(
            folder, f"energy_decay_{center_freq}Hz_{timestamp}.svg")
